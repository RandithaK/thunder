...
import {MagicLinkCallback} from '@thunderid/react';
import {FC} from 'react';
import {useLocation, useNavigate} from 'react-router';

/**
 * Props for the MagicLinkRoute component.
 */
export interface MagicLinkRouteProps {
  /**
   * Callback function called when an error occurs during Magic Link processing.
  ...
  /**
   * Callback function called when Magic Link authentication completes.
   * @param authData - The authentication data returned by the flow.
   */
...
/**
 * Handles Magic Link callback redirects for React Router applications.
 * Verifies the token on the callback route, then navigates to the next safe route.
 *
 * @example
 * ```tsx
 * <Route path="/magiclink" element={<MagicLinkRoute />} />
 * ```
 */
const MagicLinkRoute: FC<MagicLinkRouteProps> = ({
  onError,
  onNavigate,
  onSuccess,
  signInPath,
