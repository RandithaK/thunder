...
describe('TokenCallback', () => {
  ...
  it('verifies the token as an authentication flow when type=AUTHENTICATION is present', async () => {
    ...
    window.history.replaceState({}, '', '/callback?id=exec-1&applicationId=app-1&token=secret-token&type=AUTHENTICATION');
  ...
  it('redirects to sign-in with an error when required parameters are missing', async () => {
    ...
    await waitFor(() => {
      expect(onError).toHaveBeenCalledWith(expect.objectContaining({message: 'Missing executionId or token in callback URL'}));
    });
