...
- An email service must be configured in <ProductName />. See [Email Providers](../notifications/email-providers.mdx). The executor returns failure status if no email client is configured.
...
**Prerequisites:**
- The OpenID4VP service must be configured in <ProductName /> with at least one presentation definition (see the [OpenID4VP protocol guide](/docs/next/guides/guides/protocols/openid4vc/openid4vp)).
