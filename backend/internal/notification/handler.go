...
func (h *notificationSenderHandler) HandleSenderListRequest(w http.ResponseWriter, r *http.Request) {
	...
	senders, svcErr := h.mgtService.ListSenders(ctx, h.senderType)
	if svcErr != nil {
...
func getDTOFromSenderRequest(
	...
	senderDTO := common.NotificationSenderDTO{
		...
		Type:        senderType,
		Provider:    common.NotificationProviderType(providerStr),
...
func getSenderResponseFromDTO(sender *common.NotificationSenderDTO) (common.NotificationSenderResponse, error) {
	...
	for _, property := range sender.Properties {
		if sender.Type == common.NotificationSenderTypeEmail &&
			property.GetName() == common.SenderPropertySupportedChannels {
			continue
		}
