...
// testValidJWT is a valid JWT with ml_data claims containing recipient and user_id.
// nolint:gosec // G101: test data, not a real secret
var testValidJWT = "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9." +
	"eyJtbF9kYXRhIjp7InJlY2lwaWVudCI6InRlc3RAZXhhbXBsZS5jb20iLCJ1c2VyX2lkIjoidXNlci0xMjMifX0." +
	"test-signature"

var testMissingUserIDJWT = "eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9." +
	"eyJtbF9kYXRhIjogeyJyZWNpcGllbnQiOiAidGVzdEBleGFtcGxlLmNvbSJ9fQ." +
	"test-signature"

var testMismatchedUserIDJWT = "eyJhbGciOiAiSFMyNTYiLCAidHlwIjogIkpXVCJ9." +
	"eyJtbF9kYXRhIjogeyJyZWNpcGllbnQiOiAidGVzdEBleGFtcGxlLmNvbSIsICJ1c2VyX2lkIjogInVzZXItNDU2In19." +
	"test-signature"
...
func (suite *MagicLinkServiceTestSuite) SetupSuite() {
	...
	if err := initializeTestRuntime(suite.T().TempDir()); err != nil {
		suite.T().Fatal(err)
	}
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkSuccess() {
	...
	suite.mockJWTService.On("GenerateJWT",
		tokenSubject, tokenAudience, mock.Anything,
		int64(DefaultExpirySeconds), mock.Anything, jwt.TokenTypeJWT,
	).Return(testToken, testIssuedAt, nil)

	suite.mockTemplateService.On("Render", mock.Anything, template.ScenarioMagicLink, mock.Anything).
	...
	err := suite.service.SendMagicLink(context.Background(), testRecipient)
	suite.Nil(err)
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkInvalidRecipient() {
	...
	for _, tc := range tests {
		suite.Run(tc.name, func() {
			err := suite.service.SendMagicLink(context.Background(), tc.recipient)
			suite.NotNil(err)
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkUserProviderError() {
	...
	err := svc.SendMagicLink(context.Background(), testRecipient)
	suite.Nil(err)
}
...
func (suite *MagicLinkServiceTestSuite) TestSendMagicLinkEmailSendError() {
	...
	// Return a regular error, not a serviceerror.ServiceError
...
func (suite *MagicLinkServiceTestSuite) TestVerifyMagicLinkMissingUserIDClaim() {
	mockJWT := jwtmock.NewJWTServiceInterfaceMock(suite.T())
	mockUserSvc := userprovidermock.NewUserProviderInterfaceMock(suite.T())
	mockEmail := emailmock.NewEmailClientInterfaceMock(suite.T())
	mockTemplate := templatemock.NewTemplateServiceInterfaceMock(suite.T())
	...
	mockJWT.On("VerifyJWT", testMissingUserIDJWT, tokenAudience, mock.Anything).Return(nil)

	result, err := svc.VerifyMagicLink(context.Background(), testMissingUserIDJWT)
	suite.Nil(result)
	suite.NotNil(err)
